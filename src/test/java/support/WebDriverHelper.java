...
public class WebDriverHelper {
	...
	public static Response GetRequest(String params) {
		Response response = given().contentType(ContentType.ANY).relaxedHTTPSValidation().log().all().when().get(params)
	...
	public static void waitUntilInvisibilityOfElementLocated(String elementLocator) {
		if (elementLocator.startsWith("//")) {
			new WebDriverWait(driver, Duration.ofSeconds(30))
					.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(elementLocator)));
		} else if (elementLocator.startsWith(".")) {
			elementLocator = elementLocator.replace(".", "");
			new WebDriverWait(driver, Duration.ofSeconds(30))
					.until(ExpectedConditions.invisibilityOfElementLocated(By.className(elementLocator)));
		}
	}
	...
	public void waitUntilPresenceOfElement(By element, int timeout) {
		...
		wait = new WebDriverWait(driver, Duration.ofSeconds(30))
				.ignoreAll(ImmutableList.of(NoSuchElementException.class, TimeoutException.class));
	...
	public void waitUntilAttributeToBePresent(WebElement element, String attribute, String value, int timeout,
											  int pollingTime) {
		wait = new FluentWait<>(driver).withTimeout(Duration.ofSeconds(timeout))
				.pollingEvery(Duration.ofSeconds(pollingTime));
	...
	public String getLocatorStringFromWebElement(WebElement element) {
		...
		// [[FirefoxDriver: firefox on WINDOWS (4520a9a1-c58b-4b34-ae3e-48613e6e7005)] -> xpath:
	...
	public void waitForAnElementTextToBeChanged(WebElement element, String expectedText, int totalWaitTime,
												int pageRefreshTimeInSeconds) {
